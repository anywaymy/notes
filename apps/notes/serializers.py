# Определяет сериализаторы дял модели Note
...
class NoteSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value): # по сути value это то, что дал нам человек
        # Проверка наличия заголовка
        if not value.strip():
            raise serializers.ValidationError('Заголовок не может быть пустым')
        return escape(value.strip())

    def validate_content(self, value): # по сути value это то, что дал нам человек
